...
bool     legEnabled[6] = {true, true, true, true, true, true};
float    mpuPitch = 0.0f;
float    mpuRoll = 0.0f;
...
bool isFaultTolerant() {
  for (uint8_t i = 0; i < 6; i++) {
    if (!legEnabled[i]) return true;
  }
  return false;
}

void faultTolerantStep(uint8_t swingId, float fwdDir, float turnDir) {
  if (!legEnabled[swingId]) return;

  uint8_t activeCount = 0;
  for (uint8_t i = 0; i < 6; i++) {
    if (legEnabled[i]) activeCount++;
  }
  if (activeCount < 3) return;

  float stepScale = SWING;
  
  float ss[6];
  for (uint8_t i = 0; i < 6; i++) { ss[i] = cxPos[i]; }
  ...
  for (uint8_t s = 0; s <= ISTEPS; s++) {
    ...
    for (uint8_t i = 0; i < 6; i++) {
      ...
      if (i == swingId) {
        target_cx = (fwdDir * stepScale) + (i < 3 ? -turnDir * stepScale : turnDir * stepScale);
        cxPos[i]  = ss[i] + (target_cx - ss[i]) * t;
        setLeg(i, cxPos[i], fs, TB_STAND);
      } else {
        float pushDist = (2.0f * stepScale) / max(1, activeCount - 1);
        float push_target = ss[i] - (fwdDir * pushDist) - (i < 3 ? -turnDir * pushDist : turnDir * pushDist);
        push_target = constrain(push_target, -38.0f, 38.0f);
        
        cxPos[i] = ss[i] + (push_target - ss[i]) * t;
        setLeg(i, cxPos[i], FM_STAND, TB_STAND);
      }
    }
    svcDelay(sd / 2);
  }
...
void faultTolerantCycle(float fwdDir, float turnDir) {
  const uint8_t WAVE_ORD[6] = {2, 5, 1, 4, 0, 3};
  for (uint8_t n = 0; n < 6; n++) {
    uint8_t i = WAVE_ORD[n];
    if (legEnabled[i]) {
      faultTolerantStep(i, fwdDir, turnDir);
      if (!isActive) return;
    }
  }
}
...
void walkCycle(int8_t dir) {
  if (isFaultTolerant()) { faultTolerantCycle(dir, 0); return; }
  tripodStep(0, dir);
  if (!isActive) return;
  tripodStep(1, dir);
}
...
void turnCycle(int8_t td) {
  if (isFaultTolerant()) { faultTolerantCycle(0, td); return; }
  tripodTurnStep(0, td);
  if (!isActive) return;
  tripodTurnStep(1, td);
}
...
void crabCycle(int8_t dir) {
  if (isFaultTolerant()) return; // Crab disabled in fault tolerance
  crabStep(0, dir);
  if (!isActive) return;
  crabStep(1, dir);
}
...
void diagCycle(int8_t fwdDir, int8_t sideDir) {
  if (isFaultTolerant()) { faultTolerantCycle(fwdDir, 0); return; } // fallback to forward
...
void processCmd(char cmd) {
  switch (cmd) {
    ...
    case 'U': case 'u':
      ...
      curMode = FAULT_TOLERANCE; isActive = true;
      setStatus("MODE: FAULT TOLERANCE");
      break;
    case 'F': case 'f':
      wakeFlag = danceFlag = false;
...
/* ══════════════════ MODAL ══════════════════ */
.ft-modal{position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:100;
 display:none;place-items:center;padding:20px;backdrop-filter:blur(4px)}
.ft-modal.open{display:grid}
.ft-card{background:var(--surface);border:1px solid var(--border);padding:20px;
 border-radius:4px;width:min(400px, 100%);box-shadow:0 0 30px rgba(0,255,204,0.15)}
.ft-title{font-size:0.8rem;color:var(--neon);margin-bottom:15px;letter-spacing:2px;text-align:center}
.ft-grid{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:20px}
.ft-leg{padding:10px;border:1px solid var(--border2);background:rgba(0,170,255,0.05);
 color:var(--text);font-family:'Share Tech Mono',monospace;font-size:0.65rem;
 text-align:center;cursor:pointer;transition:all 0.15s;border-radius:2px}
.ft-leg.disabled{border-color:rgba(255,58,124,0.5);color:var(--neon3);background:rgba(255,58,124,0.1)}
.ft-leg:hover{border-color:var(--neon)}
.ft-close{width:100%;padding:10px;background:linear-gradient(135deg,rgba(0,255,204,0.1),transparent);
 border:1px solid var(--neon);color:var(--neon);font-family:'Rajdhani',sans-serif;
 ...
 <div class="main">
  ...
  <div class="center-panel">
   ...
   <div class="g4">
    ...
    <button class="btn btn-y" onclick="openFTModal()" style="padding:0">🛠 FTOLERANCE</button>
   </div>
...
<!-- FAULT TOLERANCE MODAL -->
<div class="ft-modal" id="ftModal">
 <div class="ft-card">
  <div class="ft-title">🛠 FAULT TOLERANCE: DISABLE LEGS</div>
  <div class="ft-grid">
   <div class="ft-leg" id="ftL0" onclick="toggleLeg(0)">FL (ENABLED)</div>
   <div class="ft-leg" id="ftL3" onclick="toggleLeg(3)">FR (ENABLED)</div>
   <div class="ft-leg" id="ftL1" onclick="toggleLeg(1)">ML (ENABLED)</div>
   <div class="ft-leg" id="ftL4" onclick="toggleLeg(4)">MR (ENABLED)</div>
   <div class="ft-leg" id="ftL2" onclick="toggleLeg(2)">BL (ENABLED)</div>
   <div class="ft-leg" id="ftL5" onclick="toggleLeg(5)">BR (ENABLED)</div>
  </div>
  <button class="ft-close" onclick="closeFTModal()">CLOSE</button>
 </div>
</div>
...
// ── Fault Tolerance ──
let ftLegs = [true, true, true, true, true, true];
...
function toggleLeg(id) {
  ...
  if (ftLegs[id]) {
    ...
    el.textContent = el.textContent.replace('DISABLED', 'ENABLED');
  } else {
    el.classList.add('disabled');
    el.textContent = el.textContent.replace('ENABLED', 'DISABLED');
  }
  fetch('/set_leg?id='+id+'&en='+(ftLegs[id]?'1':'0'));
}
...
// ════════════════ Fault Tolerance ════════════════════════════════
void doFaultTolerance() {
  mpuWire.beginTransmission(MPU_ADDR);
  mpuWire.write(ACCEL_XOUT_H);
  mpuWire.endTransmission(false);
  mpuWire.requestFrom(MPU_ADDR, 14, true);

  int16_t ax = (mpuWire.read() << 8) | mpuWire.read();
  int16_t ay = (mpuWire.read() << 8) | mpuWire.read();
  int16_t az = (mpuWire.read() << 8) | mpuWire.read();
  mpuWire.read(); mpuWire.read(); // Skip temp
  mpuWire.read(); mpuWire.read(); // Skip gyro
  mpuWire.read(); mpuWire.read();
  mpuWire.read(); mpuWire.read();

  // Simple Pitch and Roll calculation
  float pitch = -(atan2(ax, sqrt(ay * ay + az * az)) * 180.0 / PI);
  float roll  = (atan2(ay, az) * 180.0 / PI);

  // Exponential smoothing
  mpuPitch = 0.8f * mpuPitch + 0.2f * pitch;
  mpuRoll  = 0.8f * mpuRoll  + 0.2f * roll;

  // Controller gains
  float pGainPitch = 0.5f; 
  float pGainRoll  = 0.5f;

  for (uint8_t i = 0; i < 6; i++) {
    if (!legEnabled[i]) {
      // Lift the disabled leg high completely to the top to simulate removal
      setLeg(i, 0.0f, -45.0f, 45.0f);
    } else {
      // Calculate active leg height offset
      float fmOffset = FM_STAND;
      
      // Pitch adjustment: front legs (0, 3) vs back legs (2, 5)
      if (i == 0 || i == 3) {
        fmOffset += mpuPitch * pGainPitch;
      } else if (i == 2 || i == 5) {
        fmOffset -= mpuPitch * pGainPitch;
      }
      
      // Roll adjustment: left legs (0, 1, 2) vs right legs (3, 4, 5)
      if (i < 3) {
        fmOffset += mpuRoll * pGainRoll;
      } else {
        fmOffset -= mpuRoll * pGainRoll;
      }
      
      fmOffset = constrain(fmOffset, -35.0f, 25.0f);
      setLeg(i, 0.0f, fmOffset, TB_STAND);
    }
  }
}
...
void setupWeb() {
  ...
  httpServer.on("/set_leg", HTTP_GET, []() {
    if (!checkAuth(true)) return;
    if (httpServer.hasArg("id") && httpServer.hasArg("en")) {
      int id = httpServer.arg("id").toInt();
      bool en = (httpServer.arg("en") == "1");
      if (id >= 0 && id < 6) legEnabled[id] = en;
    }
