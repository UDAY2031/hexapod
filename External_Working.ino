...
// per side by JDIR, so both left and right fold identically.
//
// Femur swings UP/back at a large negative angle.
// The TIBIA should rotate the SAME physical way as the femur (up/back).
// On most builds the lower joint must move OPPOSITE in sign to curl the
// foot up alongside the femur, so the default tibia tuck is POSITIVE here.
//   -> If the tibia folds the WRONG way, flip the sign of TUCK_TB only
//      (make it negative). Adjust the magnitude if it under/over-rotates.
const float TUCK_FM = -42.0f;  // femur up/back
const float TUCK_TB = +45.0f;  // tibia rotates up/back to match femur
...
void svcAll() {
  ...
  readMPU();                 // passive telemetry only — see MPU block below
}
...
// ═══════════════════════════════════════════════════════════════════
//   MPU6050 — PASSIVE TELEMETRY ONLY
//   Reads accel/gyro and computes roll/pitch/yaw. These values are NOT
//   used by any motion, fault, balance, or gait code anywhere — they are
//   read and exposed (serial + /mpu endpoint) for monitoring only.
//   Self-contained: direct Wire register access, no extra library.
//   Timeout-/presence-guarded so a missing or flaky MPU can never block
//   the loop or affect the robot.
// ═══════════════════════════════════════════════════════════════════
static const uint8_t MPU_ADDR = 0x68;
bool     mpu_ok = false;                 // detected at boot?
float    mpuRoll = 0, mpuPitch = 0, mpuYaw = 0;     // degrees
float    mpuAx = 0, mpuAy = 0, mpuAz = 0;           // g
float    mpuGx = 0, mpuGy = 0, mpuGz = 0;           // deg/s
float    mpu_gbiasX = 0, mpu_gbiasY = 0, mpu_gbiasZ = 0;
uint32_t mpu_last_us = 0;
uint32_t mpu_print_ms = 0;
const float MPU_ALPHA = 0.96f;           // complementary filter blend
...
void initMPU() {
  // presence check via WHO_AM_I
  Wire.beginTransmission(MPU_ADDR);
  Wire.write(0x75);
  if (Wire.endTransmission(false) != 0) { mpu_ok = false; Serial.println("MPU: not responding"); return; }
  Wire.requestFrom(MPU_ADDR, (uint8_t)1);
  uint8_t who = Wire.available() ? Wire.read() : 0xFF;
  mpu_ok = (who == 0x68 || who == 0x70 || who == 0x72 || who == 0x71 || who == 0x73);
  if (!mpu_ok) { Serial.print("MPU: WHO_AM_I=0x"); Serial.println(who, HEX); Serial.println("MPU: absent"); return; }
  ...
  // quick gyro-bias capture (robot should be still at boot)
  long sx = 0, sy = 0, sz = 0; const int N = 80;
  for (int i = 0; i < N; i++) {
    ...
    sx += gx; sy += gy; sz += gz; delay(3);
  }
  mpu_gbiasX = sx / (float)N; mpu_gbiasY = sy / (float)N; mpu_gbiasZ = sz / (float)N;
  mpu_last_us = micros();
  Serial.println("MPU: online (telemetry only)");
}

// Throttled read (~50 Hz). Pure measurement; touches no robot state.
void readMPU() {
  if (!mpu_ok) return;
  uint32_t now = micros();
  if (now - mpu_last_us < 20000) return;          // ~50 Hz cap
  float dt = (now - mpu_last_us) / 1e6f;
  mpu_last_us = now;

  Wire.beginTransmission(MPU_ADDR); Wire.write(0x3B); 
  if (Wire.endTransmission(false) != 0) return;   // bus hiccup -> skip, never block
  ...
  mpuAx = ax / 16384.0f; mpuAy = ay / 16384.0f; mpuAz = az / 16384.0f;
  mpuGx = (gx - mpu_gbiasX) / 131.0f;
  mpuGy = (gy - mpu_gbiasY) / 131.0f;
  mpuGz = (gz - mpu_gbiasZ) / 131.0f;

  float accRoll  = atan2f(mpuAy, mpuAz) * 180.0f / PI;
  float accPitch = atan2f(-mpuAx, sqrtf(mpuAy*mpuAy + mpuAz*mpuAz)) * 180.0f / PI;

  mpuRoll  = MPU_ALPHA * (mpuRoll  + mpuGx * dt) + (1 - MPU_ALPHA) * accRoll;
  mpuPitch = MPU_ALPHA * (mpuPitch + mpuGy * dt) + (1 - MPU_ALPHA) * accPitch;
  mpuYaw  += mpuGz * dt;

  // periodic serial echo (read-only)
  if (millis() - mpu_print_ms > 500) {
    mpu_print_ms = millis();
    Serial.print("MPU R="); Serial.print(mpuRoll, 1);
    Serial.print(" P=");    Serial.print(mpuPitch, 1);
    Serial.print(" Y=");    Serial.print(mpuYaw, 1);
    Serial.print(" | aZ="); Serial.println(mpuAz, 2);
  }
}
...
void setupWeb() {
  ...
  // Read-only MPU telemetry endpoint. Exposes values only; nothing acts on them.
  httpServer.on("/mpu", HTTP_GET, []() {
    if (!checkAuth(true)) return;
    String j; j.reserve(220);
    j  = "{\"ok\":"; j += mpu_ok ? "true" : "false";
    j += ",\"roll\":";  j += String(mpuRoll, 2);
    j += ",\"pitch\":"; j += String(mpuPitch, 2);
    j += ",\"yaw\":";   j += String(mpuYaw, 2);
    j += ",\"ax\":";    j += String(mpuAx, 3);
    j += ",\"ay\":";    j += String(mpuAy, 3);
    j += ",\"az\":";    j += String(mpuAz, 3);
    j += ",\"gx\":";    j += String(mpuGx, 2);
    j += ",\"gy\":";    j += String(mpuGy, 2);
    j += ",\"gz\":";    j += String(mpuGz, 2);
    j += "}";
    httpServer.send(200, "application/json", j);
  });
...
void setup() {
  ...
  initMPU();   // passive telemetry only; safe if absent
