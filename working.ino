...
// ══════════════════════════ Leg Layout ═══════════════════════════
struct Leg { uint8_t cx, fm, tb; int8_t dir; const char* name; };
// dir: +1 = left side, -1 = right side (mirrors coxa direction)
const Leg LEG[6] = {
  { 0, 1, 2, +1, "L1" },  // 0 FL  Front Left
  { 3, 4, 5, +1, "L2" },  // 1 ML  Mid   Left
  { 6, 7, 8, +1, "L3" },  // 2 BL  Back  Left
  { 9,10,11, -1, "R1" },  // 3 FR  Front Right
  {12,13,14, -1, "R2" },  // 4 MR  Mid   Right
  {15,16,17, -1, "R3" }   // 5 BR  Back  Right
};
...
// Folded-leg pose (foot fully clears the ground)
const float TUCK_FM = -38.0f;  // femur up
const float TUCK_TB = -20.0f;  // tibia curled in
...
// NOTE: this is the ORIGINAL low-level write — no offsets folded in.
...
.jb{width:168px;height:168px;border-radius:50%;border:1px solid var(--br);
 ...
 position:relative;touch-action:none;cursor:grab;box-shadow:0 0 18px rgba(0,255,136,0.07),inset 0 0 22px rgba(0,0,0,0.55)}
.js{width:62px;height:62px;border-radius:50%;border:1px solid rgba(0,255,136,0.75);
 background:radial-gradient(circle at 36% 36%,rgba(0,255,136,0.52),rgba(0,255,136,0.16));
 position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);box-shadow:0 0 14px rgba(0,255,136,0.45)}
...
.dc{width:38px;height:38px;border:1px solid rgba(0,255,136,0.12);border-radius:2px;display:grid;place-items:center;font-size:0.85rem;color:var(--m);transition:all 0.15s}
.dc.on{border-color:var(--g);background:rgba(0,255,136,0.17);color:var(--g);box-shadow:0 0 8px rgba(0,255,136,0.38)}
...
<div class="card">
 ...
 <div class="main">
  ...
  <div>
   <div class="dp">
    <div class="dc"></div><div class="dc" id="dcF">▲</div><div class="dc"></div>
    <div class="dc" id="dcL">↺</div><div class="dc dc-mid">●</div><div class="dc" id="dcR">↻</div>
    <div class="dc"></div><div class="dc" id="dcB">▼</div><div class="dc"></div>
   </div>
  </div>
 </div>
...
let lastDC=null;
function dpadOn(c){ if(lastDC)$(lastDC)?.classList.remove('on'); lastDC=null; const d=DMAP[c.toUpperCase()]; if(d){$(d)?.classList.add('on');lastDC=d;} }

async function cmd(c){
  // movement/stop pills only for single-letter movement commands
  if(['F','B','L','R','S'].includes(c.toUpperCase())){ pillOn(c); dpadOn(c); }
  try{
...
function joyPt(cx,cy){ const rect=jb.getBoundingClientRect(); let dx=cx-(rect.left+rect.width/2),dy=cy-(rect.top+rect.height/2); const d=Math.hypot(dx,dy); if(d>R){const k=R/d;dx*=k;dy*=k;} return{dx,dy}; }
function joyCmd(dx,dy){ const ax=Math.abs(dx),ay=Math.abs(dy); if(ax<TH&&ay<TH)return'S'; return ay>=ax?(dy<0?'F':'B'):(dx<0?'L':'R'); }
function applyJoy(dx,dy){ js.style.transform='translate(calc(-50% + '+dx+'px),calc(-50% + '+dy+'px))'; const c=joyCmd(dx,dy); if(c!==lastCmd){lastCmd=c;if(c!=='S')clearModeBtn();cmd(c);} }
jb.addEventListener('pointerdown',e=>{drag=true;jb.setPointerCapture(e.pointerId);jb.classList.add('active');const{dx,dy}=joyPt(e.clientX,e.clientY);applyJoy(dx,dy);},{passive:true});
jb.addEventListener('pointermove',e=>{if(!drag)return;const{dx,dy}=joyPt(e.clientX,e.clientY);applyJoy(dx,dy);},{passive:true});
function resetJoy(){drag=false;jb.classList.remove('active');js.style.transform='translate(-50%,-50%)';if(lastCmd!=='S'){lastCmd='S';cmd('S');}}
...
void loop() {
  ...
  switch (curMode) {
    case WALK_F:
      if (isActive) { if (fiveLeg) waveCycle5(+1, 0); else walkCycle(+1 * DIR_SIGN); }
      break;
    case WALK_B:
      if (isActive) { if (fiveLeg) waveCycle5(-1, 0); else walkCycle(-1 * DIR_SIGN); }
      break;
    case TURN_L:
      if (isActive) { if (fiveLeg) waveCycle5(0, -1); else turnCycle(-1); }
      break;
    case TURN_R:
      if (isActive) { if (fiveLeg) waveCycle5(0, +1); else turnCycle(+1); }
      break;
