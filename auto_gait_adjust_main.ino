/*
 ...
 ║     • ★ VISIBLE COG SHIFT: neighbor coxa rotation on fault        ║
...
// Recovery offset table: [coxa offsets, femur_bias, tibia_bias, stride_scale, turn_bias]
struct RecoveryRule {
  float cx_offset[6];
  float fm_bias;
  float tb_bias;
...
// ══════ ★ COG SHIFT TABLE — Visible coxa rotation toward disabled leg ══════
//
//  Hexapod layout (top view):
//       FRONT
//    L1 ───── R1      coxa +angle = forward sweep
//    L2 ───── R2      coxa –angle = backward sweep
//    L3 ───── R3      (normalized by cxDir for L/R mirror)
//       BACK
//
//  When leg X is disabled, its two closest neighbors rotate their coxa
//  TOWARD leg X's position. This visibly shifts the support polygon
//  to cover the missing leg area.  ~12° offset is clearly visible.

struct CogCompensation {
  uint8_t n1, n2;        // Two neighbor leg indices
  float   off1, off2;    // Coxa offset degrees for each (+ = forward, – = back)
};

const CogCompensation COG_COMP[6] = {
  // L1 disabled → L2 sweeps backward toward L1,  R1 sweeps backward toward L1
  { 1, 3, -12.0f, -10.0f },

  // L2 disabled → L1 sweeps forward toward L2, L3 sweeps backward toward L2
  { 0, 2, +10.0f, -10.0f },

  // L3 disabled → L2 sweeps forward toward L3, R3 sweeps forward toward L3
  { 1, 5, +12.0f, +10.0f },

  // R1 disabled → R2 sweeps backward toward R1,  L1 sweeps backward toward R1
  { 4, 0, -12.0f, -10.0f },

  // R2 disabled → R1 sweeps forward toward R2, R3 sweeps backward toward R2
  { 3, 5, +10.0f, -10.0f },

  // R3 disabled → R2 sweeps forward toward R3, L3 sweeps forward toward R3
  { 4, 2, +12.0f, +10.0f },
};

FaultState faultState = FAULT_NONE;
uint8_t faultedLeg = 255;
uint32_t faultDetectedTime = 0;
float faultConfidence = 0.0f;
const float FAULT_CONFIDENCE_THRESH = 0.72f;
const uint32_t FAULT_HYSTERESIS_MS = 500;
...
// Walk parameters (dynamic — adjusted during fault)
float    SWING    = 28.0f;
float    LIFT     = 22.0f;
float    FM_STAND =  0.0f;
float    TB_STAND =  0.0f;
...
// Saved normal gait params (restored on leg re-enable)
const float SWING_NORMAL   = 28.0f;
const float LIFT_NORMAL    = 22.0f;
const uint16_t STEP_NORMAL = 450;
...
void applyCogShift(uint8_t disabledLeg);
void clearCogShift();
...
void updateFaultDetection() {
  ...
  const float TILT_THRESH = 3.5f;

  if (tiltMagnitude < TILT_THRESH) {
    if (faultState == FAULT_DETECTED) {
      if (millis() - faultDetectedTime > 3000) {
  ...
  if (leg == 255 || conf < FAULT_CONFIDENCE_THRESH) return;
...
void clearRecoveryOffsets() {
  for (uint8_t i = 0; i < 6; i++) cxRecoveryOffset[i] = 0.0f;
  SWING = SWING_NORMAL;
  LIFT = LIFT_NORMAL;
  FM_STAND = 0.0f;
  TB_STAND = 0.0f;
...
// ════════════════════════ 12. ★ COG SHIFT (DEMO FEATURE) ════════════
//
//  When a leg is disabled via button, this smoothly rotates the two
//  nearest neighbor legs' coxa servos TOWARD the disabled leg.
//  The motion is clearly visible on the hardware — judges will see
//  the robot auto-adjust its stance to remain stable.
//
//  Animation: 40 steps × 15ms ≈ 600ms smooth ease-in/ease-out

void applyCogShift(uint8_t disabledLeg) {
  if (disabledLeg >= 6) return;

  // Clear any previous offsets (handles case of switching which leg is disabled)
  for (uint8_t j = 0; j < 6; j++) cxRecoveryOffset[j] = 0.0f;

  const CogCompensation& comp = COG_COMP[disabledLeg];

  Serial.println("──────────────────────────────────────");
  Serial.print(">>> COG SHIFT ACTIVE: ");
  Serial.print(LEG[disabledLeg].name);
  Serial.println(" disabled");
  Serial.print("    ");
  Serial.print(LEG[comp.n1].name);
  Serial.print(" → ");
  Serial.print(comp.off1 > 0 ? "+" : "");
  Serial.print(comp.off1, 1);
  Serial.print("°   ");
  Serial.print(LEG[comp.n2].name);
  Serial.print(" → ");
  Serial.print(comp.off2 > 0 ? "+" : "");
  Serial.print(comp.off2, 1);
  Serial.println("°");
  Serial.println("──────────────────────────────────────");

  // Reduce stride & slow step for 5-leg stability
  SWING = 22.0f;
  LIFT  = 20.0f;
  STEP_MS = 520;

  // Smooth ease-in-out animation
  const uint8_t ANIM_STEPS = 40;
  for (uint8_t s = 0; s <= ANIM_STEPS; s++) {
    float t = (float)s / ANIM_STEPS;
    float e = 0.5f - 0.5f * cosf(t * PI);   // cosine ease

    cxRecoveryOffset[comp.n1] = comp.off1 * e;
    cxRecoveryOffset[comp.n2] = comp.off2 * e;

    // Physically move the neighbor servos
    if (legState[comp.n1] == LEG_ACTIVE)
      setLeg(comp.n1, cxRecoveryOffset[comp.n1], FM_STAND, TB_STAND);
    if (legState[comp.n2] == LEG_ACTIVE)
      setLeg(comp.n2, cxRecoveryOffset[comp.n2], FM_STAND, TB_STAND);

    delay(15);
  }

  Serial.println(">>> COG SHIFT COMPLETE — robot stabilized");
}
...
void clearCogShift() {
  ...
  for (uint8_t j = 0; j < 6; j++) {
    startOff[j] = cxRecoveryOffset[j];
    if (fabsf(startOff[j]) > 0.1f) hasAny = true;
  }
  if (!hasAny) return;

  Serial.println(">>> COG SHIFT: clearing offsets → returning to neutral");

  const uint8_t ANIM_STEPS = 40;
  for (uint8_t s = 0; s <= ANIM_STEPS; s++) {
    float t = (float)s / ANIM_STEPS;
    float e = 0.5f - 0.5f * cosf(t * PI);

    for (uint8_t j = 0; j < 6; j++) {
      cxRecoveryOffset[j] = startOff[j] * (1.0f - e);
      if (legState[j] == LEG_ACTIVE)
        setLeg(j, cxRecoveryOffset[j], FM_STAND, TB_STAND);
    }
    delay(15);
  }
  ...
  Serial.println(">>> GAIT RESTORED TO NORMAL");
}
...
void executeDisableLeg(uint8_t i) {
  ...
  // ── PHASE 1: Lift the failing leg off the ground ──
  ...
  // COG shift is now MANUAL — user presses the ADJUST GAIT button
  setStatus(String("LEG ") + LEG[i].name + " DISABLED - PRESS ADJUST");
}
...
void executeEnableLeg(uint8_t i) {
  ...
  // Clear any active COG shift before lowering the leg
  clearCogShift();
  delay(100);

  // ── PHASE 2: Lower the leg back to standing ──
  float cxStart = cxPos[i], fmStart = fmPos[i], tbStart = tbPos[i];
  const uint8_t STEPS = 30;
  for (uint8_t s = 0; s <= STEPS; s++) {
    float t  = (float)s / STEPS;
...
// ════════════════════════ 15. GAIT ENGINE ════════════════════════════
//
//  ★ FIXED: cxRecoveryOffset is applied to the TARGETS only.
//    Start position is taken from cxPos (which already reflects
//    the current physical position). This prevents the offset from
//    being double-applied across walk cycles.

static inline void runHalfCycle(const uint8_t* sg, const uint8_t* st,
                                const float swTgt[3], const float stTgt[3]) {
  // Capture start positions (wherever servos are right now)
  float ss[3], ts[3];
  ...
  for (uint8_t s = 0; s <= ISTEPS; s++) {
    ...
    // Swing group: legs in the air moving forward
    for (uint8_t k = 0; k < 3; k++) {
      uint8_t li = sg[k];
      if (legState[li] != LEG_ACTIVE) continue;
      float target = swTgt[k] + cxRecoveryOffset[li];   // offset biases the target
      setLeg(li, ss[k] + (target - ss[k]) * t, fs, TB_STAND);
    }

    // Stance group: legs on ground pushing backward
    for (uint8_t k = 0; k < 3; k++) {
      uint8_t li = st[k];
      if (legState[li] != LEG_ACTIVE) continue;
      float target = stTgt[k] + cxRecoveryOffset[li];
      setLeg(li, ts[k] + (target - ts[k]) * t, FM_STAND, TB_STAND);
    }
...
<div class="card">
 ...
 <div class="hint">WIFI: CHITTI-2.0 &middot; PASS: hexapod123</div>
</div>
</body></html>
)RAW";

// ════════════════════════ 19. HTML — CONTROL + VOICE ════════════════
...
.cog-box{background:rgba(0,204,255,0.06);border:1px solid rgba(0,204,255,0.35);border-radius:2px;padding:7px 9px;margin-bottom:9px;font-size:0.65rem;color:var(--g2);letter-spacing:0.8px}
.cog-box b{color:var(--g2);font-weight:700}
...
.lbtn.cog-active{background:rgba(0,204,255,0.15)!important;border-color:rgba(0,204,255,0.6)!important;color:var(--g2)!important;box-shadow:0 0 9px rgba(0,204,255,0.35)!important}
...
#voiceBtn.on{background:rgba(0,204,255,0.18)!important;border-color:rgba(0,204,255,0.7)!important;color:var(--g2)!important;box-shadow:0 0 14px rgba(0,204,255,0.35)!important;animation:vpulse 2s ease infinite}
@keyframes vpulse{0%,100%{box-shadow:0 0 8px rgba(0,204,255,0.2)}50%{box-shadow:0 0 20px rgba(0,204,255,0.55)}}
.voice-sts{text-align:center;font-size:0.6rem;color:var(--m);letter-spacing:0.8px;margin-top:5px;padding:4px;min-height:18px}
.voice-heard{color:var(--g2)!important;font-weight:700}
</style></head><body>
<div class="card">
 <div class="hdr">
 ...
 <div class="fault-box" id="faultBox" style="display:none">
  <b>&#9888; FAULT:</b> <span id="faultText">-</span> (confidence: <span id="faultConf">-</span>%)
 </div>

 <div class="cog-box" id="cogBox" style="display:none">
  <b>&#8634; COG SHIFT ACTIVE:</b> <span id="cogText">-</span>
 </div>
 ...
 <button id="voiceBtn" onclick="toggleVoice()">&#127908; VOICE COMMAND</button>
 <div class="voice-sts" id="voiceText">TAP TO START &middot; SAY: FORWARD / BACK / LEFT / RIGHT / STOP</div>
 ...
 <div class="legbox">
  ...
  <div class="lghelp">TAP TO DISABLE &middot; NEIGHBORS AUTO-ADJUST COXA (COG SHIFT)</div>
 </div>
 ...
 <div class="lbl">&#9733; gait adjust (manual)</div>
 <div class="g2">
  <button id="adjBtn" onclick="adjustGait()" style="background:rgba(0,204,255,0.1);border-color:rgba(0,204,255,0.5);color:#00ccff;font-size:0.85rem">&#9733; ADJUST GAIT</button>
  <button id="rstBtn" onclick="resetGait()" style="font-size:0.75rem">RESET GAIT</button>
 </div>
 <div style="text-align:center;font-size:0.55rem;color:#3a7a5a;letter-spacing:0.8px;margin-top:4px">DISABLE A LEG FIRST, THEN PRESS ADJUST</div>
...
async function toggleLeg(i){try{const r=await fetch('/leg?i='+i,{cache:'no-store'});if(r.status===401){location='/';return;}$('sts').textContent=await r.text();setTimeout(poll,600);}catch(e){$('sts').textContent='ERR: '+e.message;}}
function toggleAutoRecovery(){sendCmd('x');}
async function adjustGait(){$('sts').textContent='ADJUSTING...';try{const r=await fetch('/adjust',{cache:'no-store'});if(r.status===401){location='/';return;}$('sts').textContent=await r.text();setTimeout(poll,300);}catch(e){$('sts').textContent='ERR: '+e.message;}}
async function resetGait(){$('sts').textContent='RESETTING...';try{const r=await fetch('/reset',{cache:'no-store'});if(r.status===401){location='/';return;}$('sts').textContent=await r.text();setTimeout(poll,300);}catch(e){$('sts').textContent='ERR: '+e.message;}}

// ── State polling ──
async function poll(){try{const r=await fetch('/state',{cache:'no-store'});if(r.status===401){location='/';return;}const j=await r.json();
$('sts').textContent=j.status;
for(let i=0;i<6;i++){const b=$('lg'+i);if(!b)continue;b.classList.remove('dis','cog-active');if(j.legs[i]===0)b.classList.add('dis');}
// Highlight COG-shifted legs
if(j.cogShift){for(let i=0;i<6;i++){if(j.legs[i]===1 && Math.abs(j.cogShift[i])>0.5){$('lg'+i)?.classList.add('cog-active');}}}
$('diagL').textContent=j.pcaL?'OK':'X';$('diagR').textContent=j.pcaR?'OK':'X';$('diagM').textContent=j.mpu?'OK':'X';
$('diagL').className=j.pcaL?'ok':'bad';$('diagR').className=j.pcaR?'ok':'bad';$('diagM').className=j.mpu?'ok':'bad';
if(j.fault>=0){$('faultBox').style.display='block';$('faultText').textContent=legNames[j.fault];$('faultConf').textContent=(j.faultConf*100).toFixed(0);}else{$('faultBox').style.display='none';}
// COG shift display
let hasCog=false;if(j.cogShift){let txt='';for(let i=0;i<6;i++){if(Math.abs(j.cogShift[i])>0.5){if(txt)txt+=', ';txt+=legNames[i]+(j.cogShift[i]>0?' +':' ')+j.cogShift[i].toFixed(1)+'°';hasCog=true;}}if(hasCog){$('cogBox').style.display='block';$('cogText').textContent=txt;}else{$('cogBox').style.display='none';}}else{$('cogBox').style.display='none';}
$('autoRecBtn').textContent=j.autoRecovery?'Auto-Recovery: ON':'Auto-Recovery: OFF';
}catch(e){}}
...
function startVoice(){
 ...
 if(!SR){$('voiceText').textContent='NOT SUPPORTED — USE CHROME BROWSER';return;}
 recognition=new SR();
 ...
 recognition.onresult=function(e){
  ...
  $('voiceText').textContent='\u{1F3A4} "'+text+'"';
  $('voiceText').className='voice-sts voice-heard';
  setTimeout(()=>{$('voiceText').className='voice-sts';},2000);
  if(text.includes('forward')||text.includes('ahead')||text.includes('go'))mv('F','bF');
  else if(text.includes('back')||text.includes('reverse')||text.includes('retreat'))mv('B','bB');
  else if(text.includes('left'))mv('L','bL');
  else if(text.includes('right'))mv('R','bR');
  else if(text.includes('stop')||text.includes('halt')||text.includes('freeze'))mv('S','bS');
  else if(text.includes('fast')||text.includes('speed up'))setSpeed('3','spd3');
  else if(text.includes('slow'))setSpeed('1','spd1');
  else if(text.includes('normal'))setSpeed('2','spd2');
  else $('voiceText').textContent='\u2753 "'+text+'" (say: forward/back/left/right/stop)';
 };
 recognition.onerror=function(e){if(e.error!=='no-speech')$('voiceText').textContent='MIC ERR: '+e.error;};
 recognition.onend=function(){if(voiceActive){try{recognition.start();}catch(ex){}}};
 try{recognition.start();voiceActive=true;$('voiceBtn').classList.add('on');$('voiceBtn').textContent='\u{1F3A4} LISTENING...';$('voiceText').textContent='SPEAK: FORWARD / BACK / LEFT / RIGHT / STOP / FAST / SLOW';}
 catch(e){$('voiceText').textContent='ERR: '+e.message;}
}
...
function stopVoice(){
 ...
 $('voiceBtn').textContent='\u{1F3A4} VOICE COMMAND';
 $('voiceText').textContent='TAP TO START';
}
</script>
</body></html>
)RAW";
...
.tab{flex:1;padding:7px;text-align:center;font-size:0.7rem;letter-spacing:1.5px;text-decoration:none;color:var(--m);border:1px solid rgba(0,255,136,0.2);border-radius:2px;text-transform:uppercase;background:rgba(0,0,0,0.4)}
.tab.active{background:rgba(0,255,136,0.13);color:var(--g);border-color:var(--g)}
.lbl{font-size:0.59rem;color:var(--m);letter-spacing:2px;text-transform:uppercase;margin:12px 0 6px;display:flex;align-items:center;gap:6px}
.lbl::after{content:'';flex:1;height:1px;background:rgba(0,255,136,0.1)}
table{width:100%;border-collapse:collapse;font-size:0.62rem;margin-bottom:8px}
th{background:rgba(0,255,136,0.08);color:var(--g);padding:6px 5px;text-align:center;letter-spacing:1.5px;font-size:0.55rem;border:1px solid rgba(0,255,136,0.15)}
td{padding:5px;text-align:center;border:1px solid rgba(0,255,136,0.1);color:var(--t)}
tr:nth-child(even) td{background:rgba(0,0,0,0.25)}
.hi{color:var(--g2);font-weight:700}
.info{font-size:0.6rem;color:var(--m);padding:7px 9px;background:rgba(0,255,136,0.04);border:1px solid rgba(0,255,136,0.13);border-radius:2px;line-height:1.5;margin-bottom:8px}
button{all:unset;display:block;width:100%;text-align:center;padding:11px 6px;border:1px solid rgba(0,255,136,0.26);border-radius:2px;font-family:inherit;font-size:0.78rem;font-weight:700;letter-spacing:1.5px;color:var(--t);background:rgba(0,0,0,0.55);cursor:pointer;text-transform:uppercase}
button:hover{background:rgba(0,255,136,0.09);border-color:var(--g);box-shadow:0 0 9px rgba(0,255,136,0.2)}
button:active{transform:scale(0.97)}
</style></head><body>
<div class="card">
 ...
 <div class="lbl">fault detection signatures (MPU)</div>
 <table>
  <tr><th>LEG</th><th>ROLL &mu;</th><th>ROLL &sigma;</th><th>PITCH &mu;</th><th>PITCH &sigma;</th><th>PHYSICS</th></tr>
  <tr><td>L1</td><td class="hi">+8.0</td><td>1.5</td><td class="hi">+5.0</td><td>1.2</td><td>front-left ++</td></tr>
  <tr><td>L2</td><td class="hi">+15.0</td><td>2.0</td><td>+0.5</td><td>1.5</td><td>mid-left +~0</td></tr>
  <tr><td>L3</td><td class="hi">+8.5</td><td>1.4</td><td class="hi">-4.8</td><td>1.3</td><td>back-left +-</td></tr>
  <tr><td>R1</td><td class="hi">-8.2</td><td>1.5</td><td class="hi">+4.9</td><td>1.2</td><td>front-right -+</td></tr>
  <tr><td>R2</td><td class="hi">-15.5</td><td>2.1</td><td>+0.3</td><td>1.6</td><td>mid-right -~0</td></tr>
  <tr><td>R3</td><td class="hi">-8.6</td><td>1.3</td><td class="hi">-5.1</td><td>1.4</td><td>back-right --</td></tr>
 </table>

 <div class="lbl">&#9733; COG shift table (demo feature)</div>
 <div class="info">
  When a leg is <b>disabled</b>, its two nearest neighbor legs <b>automatically rotate their coxa
  servos toward the disabled leg</b>. This visibly shifts the center-of-gravity to keep the robot stable.
  The animation takes ~600ms with smooth easing. Stride is reduced to 22&deg; and step time slowed to 520ms.
 </div>
 <table>
  <tr><th>DISABLED</th><th>NEIGHBOR 1</th><th>OFFSET 1</th><th>NEIGHBOR 2</th><th>OFFSET 2</th></tr>
  <tr><td>L1</td><td>L2</td><td class="hi">+12.0&deg;</td><td>R1</td><td class="hi">+10.0&deg;</td></tr>
  <tr><td>L2</td><td>L1</td><td class="hi">-10.0&deg;</td><td>L3</td><td class="hi">+10.0&deg;</td></tr>
  <tr><td>L3</td><td>L2</td><td class="hi">-12.0&deg;</td><td>R3</td><td class="hi">-10.0&deg;</td></tr>
  <tr><td>R1</td><td>R2</td><td class="hi">+12.0&deg;</td><td>L1</td><td class="hi">+10.0&deg;</td></tr>
  <tr><td>R2</td><td>R1</td><td class="hi">-10.0&deg;</td><td>R3</td><td class="hi">+10.0&deg;</td></tr>
  <tr><td>R3</td><td>R2</td><td class="hi">-12.0&deg;</td><td>L3</td><td class="hi">-10.0&deg;</td></tr>
 </table>

 <div class="lbl">voice commands</div>
 <table>
  <tr><th>SAY</th><th>ACTION</th></tr>
  <tr><td>"forward" / "go" / "ahead"</td><td>Walk forward</td></tr>
  <tr><td>"back" / "reverse" / "retreat"</td><td>Walk backward</td></tr>
  <tr><td>"left"</td><td>Turn left</td></tr>
  <tr><td>"right"</td><td>Turn right</td></tr>
  <tr><td>"stop" / "halt" / "freeze"</td><td>Stop movement</td></tr>
  <tr><td>"fast" / "speed up"</td><td>Fast speed</td></tr>
  <tr><td>"slow"</td><td>Slow speed</td></tr>
  <tr><td>"normal"</td><td>Normal speed</td></tr>
 </table>
 <div class="info">Voice control uses the <b>Web Speech API</b> (Chrome required). Tap the microphone
 button on the Control page to activate. Works best in quiet environments.</div>

 <div class="lbl">gait parameters</div>
 <table>
  <tr><th>PARAM</th><th>NORMAL</th><th>FAULT MODE</th></tr>
  <tr><td>SWING</td><td>28&deg;</td><td>22&deg;</td></tr>
  <tr><td>LIFT</td><td>22&deg;</td><td>20&deg;</td></tr>
  <tr><td>STEP TIME</td><td>450ms</td><td>520ms</td></tr>
  <tr><td>FM_STAND</td><td>0&deg;</td><td>0&deg;</td></tr>
  <tr><td>TB_STAND</td><td>0&deg;</td><td>0&deg;</td></tr>
 </table>
...
void handleLoginPost() {
  ...
  if (pw == UI_PASS) {
    ...
    authExp = millis() + 3600000;   // 1 hour session
    httpServer.sendHeader("Set-Cookie", "HEX=1; Path=/");
    httpServer.sendHeader("Location", "/");
    httpServer.send(302, "text/plain", "");
  } else {
    String html = FPSTR(LOGIN_HTML);
    html.replace("%ERR%", "<div class=\"err\">INVALID ACCESS CODE</div>");
    httpServer.send(200, "text/html", html);
  }
}

void handleLogout() {
  httpServer.sendHeader("Set-Cookie", "HEX=0; Path=/; Max-Age=0");
...
void handleRoot() {
  if (!checkAuth()) return;
  httpServer.send(200, "text/html", FPSTR(CTRL_HTML));
}

void handleCmd() {
  if (!checkAuth(true)) return;
  String c = httpServer.arg("c");
  if (c.length() > 0) processCmd(c.charAt(0));
...
void handleLeg() {
  ...
  if (i >= 0 && i < 6) toggleLeg(i);
  httpServer.send(200, "text/plain", sysStatus);
}

void handleState() {
  if (!checkAuth(true)) return;
  String json = "{\"status\":\"" + sysStatus + "\",\"legs\":[";
  for (int i = 0; i < 6; i++) {
    if (i > 0) json += ",";
    json += (legState[i] == LEG_ACTIVE) ? "1" : "0";
  }
  json += "],\"pcaL\":";  json += pcaL_ok ? "1" : "0";
  json += ",\"pcaR\":";   json += pcaR_ok ? "1" : "0";
  json += ",\"mpu\":";    json += mpu_ok  ? "1" : "0";
  json += ",\"fault\":";  json += String(faultedLeg == 255 ? -1 : (int)faultedLeg);
  json += ",\"faultConf\":"; json += String(faultConfidence, 2);
  json += ",\"autoRecovery\":"; json += autoRecoveryEnabled ? "1" : "0";

  // ★ Include COG shift offsets so the UI can display them
  json += ",\"cogShift\":[";
  for (int i = 0; i < 6; i++) {
    if (i > 0) json += ",";
    json += String(cxRecoveryOffset[i], 1);
  }
  json += "]";
...
void handleTelemetryPage() {
  if (!checkAuth()) return;
  httpServer.send(200, "text/html", FPSTR(TELEM_HTML));
}

void handleTelemetryJson() {
  if (!checkAuth(true)) return;
  String json = "{\"roll\":";  json += String(roll_deg(), 2);
  json += ",\"pitch\":";       json += String(pitch_deg(), 2);
  json += ",\"yaw\":";         json += String(yaw_deg(), 2);
  json += ",\"history\":[";
  for (int i = 0; i < TELEM_LEN; i++) {
    uint16_t idx = (telem_idx + i) % TELEM_LEN;
    if (i > 0) json += ",";
    json += "{\"roll\":";  json += String(telem_roll[idx], 1);
    json += ",\"pitch\":"; json += String(telem_pitch[idx], 1);
    json += "}";
  }
  json += "]}";
  httpServer.send(200, "application/json", json);
...
void handleConfig() {
  if (!checkAuth()) return;
  httpServer.send(200, "text/html", FPSTR(CONFIG_HTML));
}

// ★ Manual ADJUST GAIT button handler
void handleAdjust() {
  if (!checkAuth(true)) return;
  // Find first disabled leg
  uint8_t disLeg = 255;
  for (uint8_t i = 0; i < 6; i++) {
    if (legState[i] == LEG_DISABLED) { disLeg = i; break; }
  }
  if (disLeg == 255) {
    httpServer.send(200, "text/plain", "NO DISABLED LEG — disable one first");
    return;
  }
  httpServer.send(200, "text/plain", String("ADJUSTING FOR ") + LEG[disLeg].name + "...");
  applyCogShift(disLeg);
  setStatus(String("COG ADJUSTED FOR ") + LEG[disLeg].name);
}

// ★ Manual RESET GAIT button handler
void handleReset() {
  if (!checkAuth(true)) return;
  httpServer.send(200, "text/plain", "RESETTING GAIT...");
  clearCogShift();
  setStatus("GAIT RESET TO NORMAL");
}

void handleCaptive() {
  httpServer.sendHeader("Location", "http://192.168.4.1/login");
  httpServer.send(302, "text/plain", "");
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("===================================================");
  Serial.println("  CHITTI 2.0 — Autonomous Fault-Tolerant Hexapod");
  Serial.println("  COG Shift + Voice Commands v2.0");
  Serial.println("===================================================");
  Serial.println();
  ...
  if (pcaL_ok) { pcaL.begin(); pcaL.setPWMFreq(PWM_FREQ); delay(10); }
  if (pcaR_ok) { pcaR.begin(); pcaR.setPWMFreq(PWM_FREQ); delay(10); }
  ...
  if (i2cPing(0x68)) {
    ...
    if (mpu_ok) {
      ...
      Serial.println("MPU6050: connection test FAILED — running without MPU");
      mpu_ok = false;
    }
  } else {
    Serial.println("MPU6050: NOT CONNECTED — running without MPU (OK)");
    mpu_ok = false;
  }
  ...
  httpServer.on("/login",          HTTP_GET,  handleLoginGet);
  httpServer.on("/login",          HTTP_POST, handleLoginPost);
  httpServer.on("/logout",                    handleLogout);
  httpServer.on("/",                          handleRoot);
  httpServer.on("/cmd",                       handleCmd);
  httpServer.on("/leg",                       handleLeg);
  httpServer.on("/state",                     handleState);
  httpServer.on("/telemetry",                 handleTelemetryPage);
  httpServer.on("/telemetry.json",            handleTelemetryJson);
  httpServer.on("/mpu/zero",                  handleMpuZero);
  httpServer.on("/config",                    handleConfig);
  httpServer.on("/adjust",                    handleAdjust);
  ...
  if (pcaL_ok || pcaR_ok) {
    smoothStartup();
    Serial.println("Servos: startup pose complete");
  }

  setStatus("READY");
  Serial.println();
  Serial.println("╔═══════════════════════════════════════════════╗");
  Serial.println("║  SYSTEM READY — Connect to CHITTI-2.0 WiFi   ║");
  Serial.println("║  Open http://192.168.4.1  Password: alien     ║");
  Serial.println("╚═══════════════════════════════════════════════╝");
}
...
void loop() {
  ...
  if (isActive) {
    switch (curMode) {
      case WALK_F: walkCycle(DIR_SIGN);   break;
      case WALK_B: walkCycle(-DIR_SIGN);  break;
      case TURN_L: turnCycle(-1);         break;
      case TURN_R: turnCycle(+1);         break;
      default: break;
    }
    // If walk was stopped mid-cycle, return to idle pose
    if (!isActive) {
      holdIdle();
    }
  }
